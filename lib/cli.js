...
(async () => {
    ...
    if (flags.all || (input.length === 0 && await askOrExit({
        ...
        return laraAll({ email, password, oraLogger, dir, concurrency, file, filePath, source }).catch(errorHandler)
    ...
    laraOne(courseUrl, { email, password, oraLogger, dir, concurrency }).catch(errorHandler)
})()
