...
(async () => {
    ...
    if (flags.all || (input.length === 0 && await askOrExit({
        ...
        const source = flags.source || await askOrExit({
            ...
            choices: [
                { title: 'Browse all api (/browse/all)', description: 'Download all from browse all api', value: 'all' },
                { title: 'Search pages api (/search/{*})', description: 'Download all courses from search pages', value: 'search' },
            ],
            initial: 0
        })

        const file = flags.file || await askOrExit({
            type   : fileChoices.length ? 'confirm' : null,
        ...
        const filePath = flags.file || await askOrExit({
            ...
            choices: fileChoices,//await folderContents(path.resolve(process.cwd(), 'json')),
