...
async function downloadFromBrowseAll(crawler, opts) {
    const {file, filePath, logger, concurrency} = opts
    let cnt = 0

    //download from '/browse/all' API
    let courses = file ? require(filePath) : await crawler.getAllCoursesFromBrowseAllAPI({ ...opts, ms }) //logger.promise(crawler.getAllCoursesFromBrowseAllAPI(opts), "Finding all courses from topics..")

    //write into file courses
    if (!file) {
        $date = new Date().toISOString();
        fs.writeFileSync(`./json/courses-all-${$date}.json`, JSON.stringify(courses, null, 2), 'utf8')
        console.log(`Courses and videos are save in file: ${path.join(process.cwd(), 'json/courses-all-${data}.json')}`);
    }
...
async function downloadFromSearchPages(crawler, opts) {
    const {file, filePath, logger, concurrency} = opts
    //get all courses from search page: https://laracasts.com/search
    const coursesFromSearch = file ? require(filePath) : await crawler.getAllCoursesFromSearchAPI({ ...opts, ms });//logger.promise(crawler.getAllCoursesFromSearchAPI({ ...opts, ms }), "Finding all courses from search..")//await crawler.getAllCoursesFromSearchAPI(opts) //
    ...
    if (!file) {
        ...
        //write into file courses
        $date = new Date().toISOString();
        fs.writeFileSync(`./json/courses-search-${$date}.json`, JSON.stringify(coursesFromSearch, null, 2), 'utf8')
        console.log(`Courses and videos are save in file: ${path.join(process.cwd(), 'json/courses-search-${data}.json')}`);
    }
    let cnt = 0
