...
module.exports = class Crawler {
    ...
    async getAllCoursesFromBrowseAllAPI(opts) {
        ...
        return Promise
            ...
            .then(async courses => {
                ms.update('info', { text: `Start extracting  ${courses.length} videos from found courses ` });
                // extract videos and sanitize
                const videos = await Promise.map(courses, async (course, index) => await this.extractVideos({course, ms, index, length: courses.length}), {
                    concurrency: opts.concurrency
                });
    ...
    async getAllCoursesFromSearchAPI(opts) {
        ...
        return Promise
            ...
            .then(async (courses) => {
                ms.update('info', { text: `Start extracting  ${courses.length} videos from found courses ` });
                return await Promise.map(courses, async (course, index) => await this.extractVideos({course, ms, index, length: courses.length}), {
                    concurrency: opts.concurrency
                });
            });
    }
     ...
     */
    async extractVideos({course, ms, index, length}) {
        ...
        ms.update('info', { text: `Extracting: ${index}/${length} series ${series} - episode ${title}` });
