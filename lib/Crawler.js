...
module.exports = class Crawler {
            ...
            .then(async () => {
                ...
                const e = await Promise
                    .map(chapters, async ({ episodes }) => {
                        ...
                        logger.info(`Found videos: ${videos.length}`)
                        return videos;
                    })
                    .then(c => c.flat())
    ...
    async extractVideos({ course, ms, index, total }) {
        ...
        logger.info(`Started Extracting in extractVideos: ${index}/${total} series ${series} - episode ${title}`)
        ...
        try {
            ...
            logger.info(`URL for of download link ${url} with course vimeoID: ${course?.vimeoId}`)
        } catch (err) {
            logger.error('error with findVideoUrl:', this._laracastsUrl + course.path, '-->err:', err);
        ...
        logger.log(`Ended Extracting in extractVideos, DOWNLOAD LINK FOUND for ${this._laracastsUrl + course.path} url: ${url}`);
        //}
        ...
        logger.debug('[extractVideos] returned object:', data)
        return data;
