...
module.exports = class Crawler {
    ...
    async getAllCoursesFromSearchAPI(opts) {
        ...
        return Promise
            ...
            .then(async (meta) => {
                ...
                return await Promise
                    .map(r, async index => {
                        console.log('`${this.searchUrl}${index}`', `${this._searchUrl}${index}`);
                        ...
                        //logger.info(` --->...${json?.props?.videos?.data.episodes.length}`)
    ...
    async extractVideos({ course, ms, index, total }) {
        ...
        ms.update('info', { text: `Extracting in extractVideos: ${index}/${total} series ${series} - episode ${title}` });
