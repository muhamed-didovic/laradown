...
module.exports = class Crawler {
    ...
    static async getCourses(searchFromLocalFile) {
                ...
                (async () => {
                    return Promise
                        ...
                        .then(async () => {
                            ...
                            // console.log('json', json.props.topics);
        ...
        // console.log('22', coursesFromSearch.length, browseAllCourses.length, courses.length);
        return courses
    }
    ...
    async getAllCoursesFromSearchAPI(opts) {
        ...
        return Promise
            ...
            .then(async (meta) => {
                ...
                return await Promise
                    .map(r, async index => {
                        console.log('searching:', `${this._searchUrl}${index}`);
                        ...
                        if (!json?.props?.videos?.data) {
                            console.log('NOTHING---->', `${this._searchUrl}${index}`);
                        } else {
                            // console.log('OK---->', json?.props?.videos?.data);
                            await fs.writeFile(`./json/search-${index}.json`, JSON.stringify(json, null, 2), 'utf8')//-${Date.now()}
                        }
    ...
    async extractVideos({ course, ms, index, total }) {
        ...
        try {
            ...
            console.log('error with findVideoUrl:', this._laracastsUrl + course.path, '-->err:', err);
    ...
    async download({ opts, ms }) {
        ...
        if (!file) {
            await fs.writeFile(`./json/all.json`, JSON.stringify(courses, null, 2), 'utf8')
        } else {
            const prefix = 'courses'
            const filename = `${prefix}-${source}-${new Date().toISOString()}.json`
        ...
        await Promise.all([
            ...
            (async () => {
                ...
                await Promise.map(courses, async (course, index) => {
                    ...
                    // logger.info(`->Downloading ${path.join(dest, course.title)} ...`)
                ...
                //ms.stopAll();
                logger.succeed(`Downloaded ${cnt} videos`)
                // logger.succeed(`Downloaded videos from ${prefix} api! (total: ${cnt})`)
            })()
        ])
    }
